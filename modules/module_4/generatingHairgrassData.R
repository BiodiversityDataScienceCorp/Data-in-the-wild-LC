...
grass_data <- tibble(date =sort(rep(seq(as.Date("2021-03-01"), as.Date("2021-04-01"), by="days"), 15.5)), 
                                  ...
                                  max_windspeed_knots = signif(rtruncnorm(n = 480, mean = 12, sd = 10), digits = 4),
...
df <- df %>% 
  rename(hairgrass_density = 1, avg_summer_temp = 2,
         avg_winter_temp = 3, penguin_density_within_100m = 4,
         n_content = 5)
...
hairgrass_data <- hairgrass_data %>% 
  ...
  mutate(hairgrass_density_per_m2 = trunc(hairgrass_density)) %>% 
  mutate(penguin_density_per_5m2_within_100m = trunc(penguin_density_within_100m)) %>% 
  select(-c(hairgrass_density, penguin_density_within_100m))
