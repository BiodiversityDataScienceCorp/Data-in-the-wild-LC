...
Let's say we're interested in finding the average of all the responses. We can do this by use the `mean` function in R:
...
Now it's your turn. Given the vector below of self-reported cooking skill ranking, calculate its mean and standard deviation:
...
fishing<-teamAntarcticaData$`Rate your skill at fishing, from 1-5 (1= low, 5=high)`
...
# create a variable that contains the column values for cold tolerance
...
Do the same thing for quality of bedside manner:

```{r}
# create a variable that contains the column values for bedside manner
